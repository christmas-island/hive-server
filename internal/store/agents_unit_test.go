...
func TestGetAgent_Success(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "name", "status", "activity", "capabilities", "last_heartbeat", "registered_at", "hive_local_version", "hive_plugin_version", "token"}).
		AddRow("agent1", "Agent One", "online", "", `["memory"]`, now.Format(time.RFC3339Nano), now.Format(time.RFC3339Nano), "", "", "token-agent1")
...
func TestGetAgent_StaleHeartbeatOffline(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "name", "status", "activity", "capabilities", "last_heartbeat", "registered_at", "hive_local_version", "hive_plugin_version", "token"}).
		AddRow("agent1", "Agent One", "online", "", `[]`, stale, stale, "", "", "token-agent1")
...
func TestListAgents_Success(t *testing.T) {
	...
	now := time.Now().UTC()
	rows := sqlmock.NewRows([]string{"id", "name", "status", "activity", "capabilities", "last_heartbeat", "registered_at", "hive_local_version", "hive_plugin_version", "token"}).
		AddRow("a1", "Agent 1", "online", "", `["tasks"]`, now.Format(time.RFC3339Nano), now.Format(time.RFC3339Nano), "", "", "token-a1").
		AddRow("a2", "Agent 2", "idle", "", `[]`, now.Format(time.RFC3339Nano), now.Format(time.RFC3339Nano), "", "", "token-a2")
...
func TestHeartbeat_Success(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "name", "status", "activity", "capabilities", "last_heartbeat", "registered_at", "hive_local_version", "hive_plugin_version", "token"}).
		AddRow("agent1", "agent1", "online", "", `["tasks"]`, now.Format(time.RFC3339Nano), now.Format(time.RFC3339Nano), "", "", "token-agent1")
	mock.ExpectQuery(regexp.QuoteMeta(
		`SELECT id, name, status, activity, capabilities, last_heartbeat, registered_at, hive_local_version, hive_plugin_version, token FROM agents WHERE id = $1`,
	)).WithArgs("agent1").WillReturnRows(rows)
...
func TestHeartbeat_NilCapabilities(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "name", "status", "activity", "capabilities", "last_heartbeat", "registered_at", "hive_local_version", "hive_plugin_version", "token"}).
		AddRow("a2", "a2", "online", "", `[]`, now.Format(time.RFC3339Nano), now.Format(time.RFC3339Nano), "", "", "token-a2")
	mock.ExpectQuery(regexp.QuoteMeta(
		`SELECT id, name, status, activity, capabilities, last_heartbeat, registered_at, hive_local_version, hive_plugin_version, token FROM agents WHERE id = $1`,
...
func TestListAgents_RowsErr(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "name", "status", "activity", "capabilities", "last_heartbeat", "registered_at", "hive_local_version", "hive_plugin_version", "token"}).
		AddRow("a1", "Agent 1", "online", "", `[]`, time.Now().Format(time.RFC3339Nano), time.Now().Format(time.RFC3339Nano), "", "", "token-a1").
		RowError(0, rowsErr)
...
func TestListAgents_MultipleAgents(t *testing.T) {
	...
	now := time.Now().UTC()
	rows := sqlmock.NewRows([]string{"id", "name", "status", "activity", "capabilities", "last_heartbeat", "registered_at", "hive_local_version", "hive_plugin_version", "token"}).
		AddRow("a1", "Alpha", "online", "", `["cap1"]`, now.Format(time.RFC3339Nano), now.Format(time.RFC3339Nano), "", "", "token-a1").
		AddRow("a2", "Beta", "offline", "", `[]`, now.Add(-time.Hour).Format(time.RFC3339Nano), now.Add(-time.Hour).Format(time.RFC3339Nano), "", "", "token-a2").
		AddRow("a3", "Gamma", "idle", "", `["cap2","cap3"]`, now.Format(time.RFC3339Nano), now.Format(time.RFC3339Nano), "", "", "token-a3")
