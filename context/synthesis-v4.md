...
Single-tenant at launch. `tenant_id` columns exist on all tables for future multi-tenancy. Day-one value: a hardcoded default tenant UUID (e.g. `00000000-0000-0000-0000-000000000001` or configurable via `HIVE_TENANT_ID` env var). All queries filter by tenant_id using this default. Agent identity comes from `X-Agent-ID` header (existing pattern). No tenant provisioning, no tenant-from-token resolution — just the column and a constant.
...
**I5: ContextEngine hook contract research**
Research spike against OpenClaw 2026.3.7 source. Document the exact `assemble` hook API: what context fields are available as input, what the return format is, whether it's async-compatible (injection must not block the agent's response turn), and what `afterTurn` receives. Define the interface contract that P1 will build against. This is design-critical — the hook contract shapes P1's entire input/output interface.

- Depends on: nothing (research only)
- Produces: documented hook contract, interface types for P1
...
- **Build immediately after S2** — having real directives makes every subsequent issue's testing better
...
Docker Compose or testcontainers for CRDB + Meilisearch + Gel. E2E smoke test: ingest skill doc → decompose → inject → feedback → verify weight change. Verify sync managers, fan-out, merge logic across all three databases. Include golden directive test cases from X2 for regression testing. CI pipeline configuration.
...
All 12 original questions resolved. Remaining unknowns:

1. **Token budget default** — tune during integration testing. Configurable per-request.
2. **Feedback attribution prompt design** — how precisely can the `afterTurn` LLM judge followed/ignored/negative? Worth a design doc before P4a to validate the learning loop will actually work.
3. **Codebase pattern ingestion** — source_type=observation triggers (webhook? cron? manual?). Deferred — build the pipeline first, add observation sources later.
