...
Contextualization is done by an LLM call (Sonnet-class) that takes the raw directive, the context frame, and produces a version specific to the current situation. Based on expected Sonnet-class latency (2.5-3.5 seconds per call), the contextualization step has a generous timeout (default 10 seconds, configurable) to ensure accuracy. This is consistent with the system's "accuracy over speed" principle -- a well-contextualized directive is worth the extra seconds. If the LLM call fails or the service is unreachable, the raw directive is returned as-is. The fallback exists for resilience, not for speed optimization.
...
-- Indexes for injection and outcome queries
...
CREATE INDEX idx_injection_outcomes_injection ON injection_outcomes (injection_id, tenant_id);
...
Read path (injection):
    Expected performance characteristics (aspirational, not hard requirements):
    1. Meilisearch: semantic search for candidates (expected ~50ms)
    2. CRDB: structured filter for candidates (expected ~10ms)
    3. Gel: chain traversal for related directives (expected ~20ms)
    4. Merge, rank, select, contextualize (hive-server compute, variable --
       dominated by the Sonnet-class LLM contextualization call at 2.5-3.5s)
    These are estimates based on typical database performance at expected
    data volumes. Actual latency will depend on deployment topology, data
    size, and whether contextualization is enabled. The principle is
    ACCURACY OVER SPEED -- correct directive selection matters more than
    shaving milliseconds off retrieval.

Reconciliation:
    - Configurable interval (default: every 5 minutes): compare CRDB
      directive timestamps against Meilisearch/Gel
...
_Note: The week estimates below are rough approximations for planning purposes, not commitments. Actual timelines will depend on complexity discovered during implementation, infrastructure setup time, and iteration on the LLM decomposition prompts._

### 10.1 Phase 1: Core Engine (roughly weeks 1-3)
...
### 10.2 Phase 2: Intelligence (roughly weeks 4-6)

**Contextualization:**

- Add Sonnet-class LLM call to adapt directives to current situation
...
### 10.3 Phase 3: Refinement (roughly weeks 7-9)
...
Hive-server v5 is a behavioral knowledge engine with four capabilities:
